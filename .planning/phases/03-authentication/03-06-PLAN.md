...
```rust
emit_audit_event(
    AuditEvent::new(AuditEventType::Login, request_id, "/api/auth/login".into())
        .with_ip(ip)
        .with_user_agent(ua)
);
```
...
```rust
emit_audit_event(
    AuditEvent::new(AuditEventType::SessionCreated, request_id, "/api/auth/callback".into())
        .with_user(&user_id)
        .with_ip(ip)
);
```

In callback handler (failure):
```rust
emit_audit_event(
    AuditEvent::new(AuditEventType::LoginFailed, request_id, "/api/auth/callback".into())
...
```rust
emit_audit_event(
    AuditEvent::new(AuditEventType::Logout, request_id, "/api/auth/logout".into())
        .with_user(&user_id)
);
```

In refresh handler (success/failure):
- TokenRefresh / TokenRefreshFailed events
  </action>
