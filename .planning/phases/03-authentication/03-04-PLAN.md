...
must_haves:
  truths:
    - "Tokens are stored in encrypted HttpOnly cookies via PrivateCookieJar"
    - "CurrentUser extractor validates JWT and provides auth0_id, email, roles (UserId placeholder until Phase 4 user lookup)"
    - "Refresh token has restricted path for security"
    - "Cookie helpers return plain Cookie objects, PrivateCookieJar wrapping happens in route handlers"
  artifacts:
  ...
  key_links:
    ...
    - from: "libs/auth/src/tokens.rs"
      to: "apps/api/src/routes/auth.rs (Plan 03-05)"
      via: "set_auth_cookies returns plain Cookies, handlers wrap in PrivateCookieJar"
      pattern: "jar\\.add.*set_auth_cookies"
---
